...
(async function renderWeather(cityWeather="poznan") {
    ...
    for(i = 1; i<=7;i++)
    {
        ...
        futureHtmlWeather += `<li class="col future-weather-list"><div>${weekday[weekDayIndex]}</div><img class="img" src="img/low/${weatherDataFuture.daily[i].weather[0].icon.slice(0,2)}.png" alt="${weatherDataFuture.daily[i].weather[0].description}"><div> Dzień ${celsius(weatherDataFuture.daily[i].temp.day)}&#176</div> <div style="color:rgb(230,230,230);"> Noc ${celsius(weatherDataFuture.daily[i].temp.night)}&#176</div></li>`;
    }
